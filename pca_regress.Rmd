...
set.seed(100)
a <- Sys.time()
pca_0.4_ordered_df <- k_fold_cv(dataset=dataset,pca_ordered=0.4,ordered = TRUE)
Sys.time()-a
# Time difference of 8.124432 mins
# predictors about 17
# write.csv(pca_0.4_ordered_df, file = "pca_0.4_ordered_df.csv", row.names = FALSE)
```

```{r}
set.seed(100)
a <- Sys.time()
pca_0.4_unordered_df <- k_fold_cv(dataset=dataset,pca_unordered=0.4,ordered = FALSE)
Sys.time()-a
# Time difference of 7.25056 mins
# predictors about 17
# write.csv(pca_0.4_unordered_df, file = "results/pca_0.4_unordered_df.csv", row.names = FALSE)
```

```{r}
set.seed(100)
a <- Sys.time()
pca_0.8_unordered_df <- k_fold_cv(dataset=dataset,pca_unordered=0.8,ordered = FALSE)
Sys.time()-a
# Time difference of 1.017887 hours
# predictors about 330
# write.csv(pca_0.8_unordered_df, file = "results/pca_0.8_unordered_df.csv", row.names = FALSE)
```
