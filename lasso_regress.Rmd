---
title: "multinomial regression with lasso"
author: "Lanyu Zhang"
date: "2023-04-25"
output: pdf_document
---
...
k_fold_cv <- function(dataset,
  ...
  # 2-Dimension reduction using Lasso
  fit <- cv.glmnet(X_train %>% as.matrix(), Y_train %>% as.numeric(), alpha=alpha)
...
set.seed(100)
a <- Sys.time()
lasso_ordered_df <- k_fold_cv(dataset=dataset,ordered = TRUE)
Sys.time()-a
# Time difference of 3.117335 mins
# predictors over 1k
# write.csv(lasso_ordered_df, file = "results/lasso_ordered_df.csv", row.names = FALSE)
```

```{r}
set.seed(100)
a <- Sys.time()
lasso_unordered_df <- k_fold_cv(dataset=dataset,ordered = FALSE)
Sys.time()-a
# Time difference of 2.902945 mins
# predictors over 1k
# write.csv(lasso_unordered_df, file = "results/lasso_unordered_df.csv", row.names = FALSE)
```
