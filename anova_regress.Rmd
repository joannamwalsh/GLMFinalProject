...
set.seed(100)
a <- Sys.time()
aov_90_ordered_df <- k_fold_cv(dataset=dataset3,aov_ordered=10^(-90),ordered = TRUE)
Sys.time()-a
# Time difference of 2.920307 mins
# predictors about 12
# write.csv(aov_90_ordered_df, file = "results/dataset3_aov_90_ordered_df.csv", row.names = FALSE)
```

```{r}
set.seed(100)
a <- Sys.time()
aov_90_unordered_df <- k_fold_cv(dataset=dataset3,aov_unordered=10^(-90),ordered = FALSE)
Sys.time()-a
# Time difference of 2.116867 mins
# predictors about 12
# write.csv(aov_90_unordered_df, file = "results/dataset3_aov_90_unordered_df.csv", row.names = FALSE)
```

```{r}
set.seed(100)
a <- Sys.time()
aov_10_unordered_df <- k_fold_cv(dataset=dataset3,aov_unordered=10^(-10),ordered = FALSE)
Sys.time()-a
# Time difference of 1.050045 hours
# predictors about 450
# write.csv(aov_10_unordered_df, file = "results/dataset3_aov_10_unordered_df.csv", row.names = FALSE)
```

```{r}
set.seed(100)
a <- Sys.time()
aov_2_unordered_df <- k_fold_cv(dataset=dataset3,aov_unordered=10^(-2),ordered = FALSE)
Sys.time()-a
# Time difference of 2.444984 hours
# predictors about 873
# write.csv(aov_2_unordered_df, file = "results/dataset3_aov_2_unordered_df.csv", row.names = FALSE)
```
